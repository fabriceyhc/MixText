...
data_dir = "./prepared_datasets/" # "../../data1/fabricehc/prepared_datasets/"

def get_data(task, n_labeled_per_class, unlabeled_per_class=0, max_seq_len=256, model='bert-base-uncased', train_aug=False):
    """Read data, split the dataset, and build dataset for dataloaders.

    Arguments:
        task {str} -- name of your task (dataset name available on huggingface)
    ...
    train_dataset = load_from_disk(os.path.join(data_dir, task + '_train_' + str(n_labeled_per_class)))
    eval_dataset  = load_from_disk(os.path.join(data_dir, task + '_valid_2000'))
    test_dataset  = load_dataset(task, split='test')

    if task == "dbpedia_14":
        train_dataset = train_dataset.rename_column("content", "text")
        eval_dataset = eval_dataset.rename_column("content", "text")
    ...
    train_labels = np.array(train_dataset['label'].tolist())
    train_text = np.array(train_dataset['text'].tolist())

    eval_labels = np.array(eval_dataset['label'].tolist())
    eval_text = np.array(eval_dataset['text'].tolist())

    test_labels = np.array(test_dataset['label'].tolist())
    test_text = np.array(test_dataset['text'].tolist())

    train_unlabeled_idxs = list(range(len(train_labels)))
    ...
    train_unlabeled_dataset = loader_unlabeled(
        train_text, train_unlabeled_idxs, tokenizer, max_seq_len, None)
    val_dataset = loader_labeled(
        eval_text, test_labels, tokenizer, max_seq_len)
